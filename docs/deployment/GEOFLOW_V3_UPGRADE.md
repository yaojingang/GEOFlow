...
后续版本请看 [3.1 升级说明](GEOFLOW_V3_1_UPGRADE.md)，包括配套 Updater 0.4.0、首次维护升级及未受管站点接管路径。本文保留 v3.0.0 的历史操作记录。
...
| 当前部署 | 升级路径 |
| --- | --- |
| 已由 Updater 管理，`primary` 实例可诊断 | [路径 A](#路径-a已有-updater-管理的实例)：[安装兼容工具](#安装-updater-v030)，执行签名更新，再完成[业务数据验收](#启动新服务与业务验收) |
| 官方源码、`docker-compose.prod.yml`、本机 PostgreSQL / Redis，尚未接管 | [路径 B](#路径-b普通生产-compose-从-2x-升级)：停机升级 Core，完成[回填](#数据回填恢复流量前完成)，再[接入 Updater](#首次接管已经升级好的-core) |
...
1. 按[安装 Updater v0.3.0](#安装-updater-v030)验证并安装兼容工具。已有实例跳过首次接管步骤。
2. 在服务器检查工具和实例：
...
Updater 会验证签名目标与镜像摘要，依次执行预检、拉取、静默、备份、迁移、激活和验证。迁移或激活等受保护阶段失败时会尝试恢复。命令成功后仍需完成[数据回填](#数据回填恢复流量前完成)和[业务验收](#启动新服务与业务验收)；自动恢复应用进程不等于业务已验收，外层维护限制保留到验收结束。
