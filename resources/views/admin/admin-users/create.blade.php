...
@section('content')
    <div class="mx-auto max-w-5xl px-4 sm:px-0">
        <header class="mb-6 flex items-start gap-4 sm:mb-8">
            ...
            <div class="min-w-0">
                <h1 class="text-balance text-2xl font-bold text-gray-900">{{ __('admin.admin_users.modal_create') }}</h1>
                <p class="mt-1 max-w-3xl text-pretty text-sm leading-6 text-gray-600">{{ __('admin.admin_users.page_subtitle') }}</p>
            </div>
        </header>

        <form method="POST" action="{{ route('admin.admin-users.store') }}" class="overflow-hidden rounded-xl bg-white shadow">
            @csrf
            ...
            <div class="border-b border-gray-200 px-5 py-5 sm:px-6">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">{{ __('admin.admin_users.role_admin') }}</h2>
                        <p class="mt-1 text-pretty text-sm leading-6 text-gray-600">{{ __('admin.admin_users.create_help') }}</p>
                    </div>
                </div>
            </div>
