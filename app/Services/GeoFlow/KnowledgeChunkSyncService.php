...
class KnowledgeChunkSyncService
{
    ...
    {
        return 'You are GEOAmplify\'s knowledge-base semantic chunk planner. You only group original block indexes into chunks. Do not rewrite, summarize, translate, add facts, or return source text. Output strict JSON only.';
    }
     ...
     */
    private function generateEmbeddingsForChunks(array $chunks, ?array $embeddingMetadata, bool $requireRealEmbedding = false, ?string $documentTitle = null): array
    {
        if ($chunks === []) {
            return [];
        }
        ...
        try {
            $results = [];
            $preparedChunks = $this->formatEmbeddingDocumentInputs($chunks, $embeddingMetadata, $documentTitle);
            foreach (array_chunk($preparedChunks, $this->embeddingBatchSize(), true) as $batch) {
                $results += $this->generateEmbeddingBatch(
                    $batch,
                    $providerName,
                    $embeddingMetadata,
     ...
     * @return array<int, array{model_id:int,dimensions:int,provider:string,vector:list<float>,vector_literal:?string}>
     */
    private function generateEmbeddingBatch(array $batch, string $providerName, array $embeddingMetadata, bool $canStoreEmbeddingVector): array
    {
        try {
            ...
            if (count($batch) <= 1 || ! $this->isEmbeddingBatchSizeError($exception)) {
                throw $exception;
            }

            $results = [];
            foreach ($batch as $chunkIndex => $input) {
                $results += $this->generateEmbeddingBatch(
                    [$chunkIndex => $input],
                    $providerName,
                    $embeddingMetadata,
    ...
    private function isEmbeddingBatchSizeError(Throwable $exception): bool
    {
        ...
        return str_contains($message, 'batch')
            ...
            || str_contains($message, '最多')
            || str_contains($message, '批量');
    }
    ...
    {
        return trim((string) (KnowledgeBase::query()->whereKey($knowledgeBaseId)->value('name') ?? ''));
    }
    ...
    {
        $query = $this->normalizeGeminiEmbeddingSegment($query);

        return $this->isGeminiEmbeddingMetadata($embeddingMetadata)
    ...
    private function formatEmbeddingDocumentInputs(array $chunks, array $embeddingMetadata, ?string $documentTitle): array
    {
        ...
        $title = $this->normalizeGeminiEmbeddingSegment((string) $documentTitle);

        return array_map(function (string $chunk) use ($title): string {
            $text = $this->normalizeGeminiEmbeddingSegment($chunk);
    ...
    /**
     * @param  array{driver?:string,api_url?:string,model_name?:string}  $embeddingMetadata
     */
    private function isGeminiEmbeddingMetadata(array $embeddingMetadata): bool
    {
        return ($embeddingMetadata['driver'] ?? '') === 'gemini'
    ...
    {
        return preg_replace('/\s+/u', ' ', $this->normalizeText($value)) ?? $this->normalizeText($value);
    }
