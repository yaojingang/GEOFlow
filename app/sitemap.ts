...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const reports = process.env.DATABASE_URL
    ...
    : [];
  const researchNoteClient = (prisma as unknown as ResearchNoteClient).researchNote;
  const researchNotes = process.env.DATABASE_URL && researchNoteClient
    ? await researchNoteClient.findMany({
